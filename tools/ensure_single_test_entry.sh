...
if ! grep -q "Use ./tools/run_tests_direct.sh" "$ROOT_DIR/forge.nimble"; then
  echo "forge.nimble does not reference the canonical test runner."
  echo "Please ensure forge.nimble contains: 'Use ./tools/run_tests_direct.sh'"
  exit 2
fi
